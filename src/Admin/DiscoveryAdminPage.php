...
class DiscoveryAdminPage {
	...
	private function handle_add_seed_url(): void {
		...
		$this->set_notice( __( 'Discovery source added. Use Run small discovery when you are ready.', 'lilleprinsen-price-monitor' ) );
	}
	...
	private function handle_run_small_discovery(): void {
		...
		if ( $this->job->enqueue_manual_batch( $competitor_id ) ) {
			$this->set_notice( __( 'Small discovery has been queued.', 'lilleprinsen-price-monitor' ) );
		} else {
			$this->set_notice( __( 'Action Scheduler is not available, so discovery could not be queued.', 'lilleprinsen-price-monitor' ), 'error' );
		}
	}
