...
final class Repository {
	...
	public function get_dashboard_counts(): array {
		...
		return array(
			...
			'competitor_health'           => $this->get_competitor_health_metrics(),
		);
	}

	/**
	 * Return bounded operational health metrics for competitor discovery and checks.
	 *
	 * @return array<string, mixed>
	 */
	public function get_competitor_health_metrics( int $days = 30 ): array {
		$days   = max( 1, min( 90, absint( $days ) ) );
		$cutoff = gmdate( 'Y-m-d H:i:s', current_time( 'timestamp' ) - ( $days * DAY_IN_SECONDS ) );

		$manual_search_attempts = $this->count_where( 'logs', 'event = %s AND created_at >= %s', array( 'manual_discovery_pair_checked', $cutoff ) );
		$manual_search_matches  = $this->count_where( 'logs', 'event = %s AND created_at >= %s', array( 'manual_discovery_match_found', $cutoff ) );
		$discovery_runs         = $this->get_discovery_run_health_totals( $cutoff );
		$extraction             = $this->get_extraction_health_totals( $cutoff );
		$approvals              = $this->get_approval_health_totals( $cutoff );

		$search_attempts = $manual_search_attempts + (int) $discovery_runs['attempts'];
		$search_success  = min( $search_attempts, $manual_search_matches + (int) $discovery_runs['successes'] );

		return array(
			'window_days'             => $days,
			'search_attempts'         => $search_attempts,
			'search_successes'        => $search_success,
			'search_success_rate'     => $this->calculate_rate( $search_success, $search_attempts ),
			'extraction_attempts'     => (int) $extraction['attempts'],
			'extraction_successes'    => (int) $extraction['successes'],
			'extraction_success_rate' => $this->calculate_rate( (int) $extraction['successes'], (int) $extraction['attempts'] ),
			'timeout_failures'        => (int) $extraction['timeouts'],
			'timeout_rate'            => $this->calculate_rate( (int) $extraction['timeouts'], (int) $extraction['attempts'] ),
			'approval_total'          => (int) $approvals['total'],
			'approval_approved'       => (int) $approvals['approved'],
			'approval_rate'           => $this->calculate_rate( (int) $approvals['approved'], (int) $approvals['total'] ),
		);
	}

	/**
	 * Return bounded successful observations for competitor strategy analytics.
	 *
	 * @return array<int, array<string, mixed>>
	 */
	public function get_competitor_strategy_observations( int $days = 45, int $limit = 3000 ): array {
		$observations = $this->tables['price_observations'];
		$links        = $this->tables['competitor_links'];

		if ( ! $this->table_exists( $observations ) || ! $this->table_exists( $links ) ) {
			return array();
		}

		$days   = max( 7, min( 180, absint( $days ) ) );
		$limit  = max( 100, min( 5000, absint( $limit ) ) );
		$cutoff = gmdate( 'Y-m-d H:i:s', current_time( 'timestamp' ) - ( $days * DAY_IN_SECONDS ) );
		$rows   = $this->wpdb->get_results(
			$this->wpdb->prepare(
				"SELECT
					po.competitor_link_id,
					po.monitored_product_id,
					po.product_id,
					po.observed_price,
					po.stock_status,
					po.success,
					...
					cl.competitor_id,
					cl.competitor_name
				FROM {$observations} po
				INNER JOIN {$links} cl ON po.competitor_link_id = cl.id
				WHERE po.success = %d
					AND po.observed_price IS NOT NULL
					AND po.checked_at >= %s
				ORDER BY po.product_id ASC, po.competitor_link_id ASC, po.checked_at ASC
				LIMIT %d",
				1,
				$cutoff,
				$limit
	 ...
	 */
	private function get_discovery_run_health_totals( string $cutoff ): array {
		$tables = DiscoverySchema::table_names();
		$table  = $tables['discovery_runs'] ?? '';
		...
		$row = $this->wpdb->get_row(
			$this->wpdb->prepare(
				"SELECT COALESCE(SUM(processed_count), 0) AS attempts, COALESCE(SUM(LEAST(processed_count, GREATEST(discovered_url_count, suggestion_count))), 0) AS successes FROM {$table} WHERE updated_at >= %s",
				$cutoff
			),
			ARRAY_A
		);
		...
		return array(
			...
			'successes' => absint( $row['successes'] ?? 0 ),
		);
	}

	/**
	 * @return array{attempts:int,successes:int,timeouts:int}
	 */
	private function get_extraction_health_totals( string $cutoff ): array {
		$attempts  = 0;
		$successes = 0;
		$timeouts  = 0;

		$observations = $this->tables['price_observations'] ?? '';
		if ( '' !== $observations && $this->table_exists( $observations ) ) {
			$row = $this->wpdb->get_row(
				$this->wpdb->prepare(
					"SELECT COUNT(*) AS attempts, SUM(CASE WHEN success = 1 THEN 1 ELSE 0 END) AS successes, SUM(CASE WHEN success = 0 AND (LOWER(COALESCE(error_message, '')) LIKE %s OR LOWER(COALESCE(error_message, '')) LIKE %s OR LOWER(COALESCE(error_message, '')) LIKE %s) THEN 1 ELSE 0 END) AS timeouts FROM {$observations} WHERE checked_at >= %s",
					'%timeout%',
					'%timed out%',
					'%curl error 28%',
					$cutoff
			...
			$attempts  += absint( $row['attempts'] ?? 0 );
			$successes += absint( $row['successes'] ?? 0 );
			$timeouts  += absint( $row['timeouts'] ?? 0 );
		}

		$discovery_tables = DiscoverySchema::table_names();
		$discovered       = $discovery_tables['discovered_competitor_products'] ?? '';
		if ( '' !== $discovered && $this->table_exists( $discovered ) ) {
			$row = $this->wpdb->get_row(
				$this->wpdb->prepare(
					"SELECT COUNT(*) AS attempts, SUM(CASE WHEN failure_count = 0 AND extraction_status <> 'queued' AND (regular_price IS NOT NULL OR sale_price IS NOT NULL) THEN 1 ELSE 0 END) AS successes FROM {$discovered} WHERE last_checked_at >= %s",
					$cutoff
				),
				ARRAY_A
			);
		...
		if ( '' !== $logs && $this->table_exists( $logs ) ) {
			$timeouts += (int) $this->wpdb->get_var(
				$this->wpdb->prepare(
					"SELECT COUNT(*) FROM {$logs} WHERE created_at >= %s AND event IN ('competitor_check_failed', 'manual_discovery_price_extraction_failed', 'manual_discovery_no_match') AND (LOWER(COALESCE(message, '')) LIKE %s OR LOWER(COALESCE(context, '')) LIKE %s OR LOWER(COALESCE(context, '')) LIKE %s)",
					$cutoff,
					'%timeout%',
					'%timeout%',
					'%timed out%'
		...
		return array(
			...
			'timeouts'  => min( $timeouts, $attempts ),
		);
	}
	 ...
	 */
	private function get_approval_health_totals( string $cutoff ): array {
		$total    = 0;
		$approved = 0;

		$suggestions = $this->tables['price_suggestions'] ?? '';
		if ( '' !== $suggestions && $this->table_exists( $suggestions ) ) {
			$row = $this->wpdb->get_row(
				$this->wpdb->prepare(
					"SELECT COUNT(*) AS total, SUM(CASE WHEN status IN ('approved_dry_run', 'approved_real_update') THEN 1 ELSE 0 END) AS approved FROM {$suggestions} WHERE created_at >= %s",
					$cutoff
				),
				ARRAY_A
			);
		...
		$discovery_tables      = DiscoverySchema::table_names();
		$discovery_suggestions = $discovery_tables['discovery_match_suggestions'] ?? '';
		if ( '' !== $discovery_suggestions && $this->table_exists( $discovery_suggestions ) ) {
			$row = $this->wpdb->get_row(
				$this->wpdb->prepare(
					"SELECT COUNT(*) AS total, SUM(CASE WHEN status = 'approved' THEN 1 ELSE 0 END) AS approved FROM {$discovery_suggestions} WHERE created_at >= %s",
					$cutoff
				),
				ARRAY_A
			);
		...
		return array(
			'total'    => $total,
			'approved' => min( $approved, $total ),
		);
	}

	private function calculate_rate( int $part, int $total ): ?float {
		if ( $total <= 0 ) {
			return null;
		}
