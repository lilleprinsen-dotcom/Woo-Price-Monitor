...
class DiscoveryRepository {
	...
	/**
	 * Get selected products that do not yet have an active approved competitor link.
	 *
	 * @return array<int,object>
	 */
	public function get_enabled_products_without_competitor_links_for_matching( int $limit = 500 ): array {
		global $wpdb;

		$tables              = Schema::table_names();
		$discovery_products  = $this->table( 'discovery_products' );
		$monitored_products  = $tables['monitored_products'];
		$competitor_links    = $tables['competitor_links'];
		$limit               = max( 1, min( 500, absint( $limit ) ) );

		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT dp.* FROM {$discovery_products} dp
				LEFT JOIN {$monitored_products} mp ON mp.product_id = dp.product_id AND mp.enabled = 1
				LEFT JOIN {$competitor_links} cl ON cl.monitored_product_id = mp.id AND cl.enabled = 1
				WHERE dp.enabled = 1 AND dp.product_id > 0 AND cl.id IS NULL
				ORDER BY dp.priority DESC, COALESCE(dp.last_discovery_at, '1970-01-01 00:00:00') ASC, dp.updated_at DESC
				LIMIT %d",
				$limit
			)
		);
