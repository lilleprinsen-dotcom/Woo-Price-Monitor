...
class SkuSearchDiscoveryService {
	...
	private const STRATEGY_OPTION = 'lpm_discovery_search_strategy_stats';
	private const MAX_QUEUE_URLS = 250;

	/** Constructor. */
	public function __construct( DiscoveryUrlService $url_service, DiscoverySourceService $source_service, DiscoverySettings $settings, ?ExternalBrowserWorkerClient $external_worker = null ) {
	...
	public function discover_for_product( array $competitor, object $product ): array {
		...
		$sku_evidence  = false;
		$searched_urls = array();
		$name_queries  = ! empty( $settings['discovery_name_search_enabled'] ) ? array_values(
			array_filter(
				$this->product_name_queries( $product ),
				static fn( $name_query ) => '' !== $name_query && strtolower( $name_query ) !== strtolower( $sku )
			)
		) : array();
		$request_budget = $this->discovery_request_budget( $request_limit, count( $queries ), count( $name_queries ) );
		if ( empty( $queries ) && empty( $name_queries ) ) {
			return $this->failure( 'This selected product has no SKU, EAN/GTIN or searchable product name.', 'Missing selected product SKU, EAN/GTIN and product title.', 0, $sku, (int) ( $product->id ?? 0 ) );
		}
		$search_order  = $this->adaptive_search_order( $competitor, $queries, $name_queries );
		$name_search_ran = false;
		...
		$errors[] = 'Adaptive search order: ' . implode( ', ', $search_order );
		$errors[] = 'Discovery request budget: ' . $request_budget;
		$queries = $this->order_identifier_queries( $queries, $search_order );
		$run_worker_search = function ( bool $internal_failed ) use ( $competitor, $product, $settings, $templates, $domain, $ports, $queries, $name_queries, $request_budget, &$urls, &$source_scores, &$source_types, &$requests, &$errors, &$searched_urls ): void {
			if ( ! $this->external_worker || ! $this->external_worker->should_use_for_search( $competitor, $settings, $internal_failed ) ) {
				return;
			}

			$worker_searches = array();
			foreach ( $queries as $query ) {
				$value = trim( (string) ( $query['value'] ?? '' ) );
				if ( '' !== $value ) {
					$worker_searches[] = $value;
				}
			}
			foreach ( $name_queries as $name_query ) {
				$worker_searches[] = (string) $name_query;
			}
			$worker_searches = array_values( array_unique( array_filter( $worker_searches ) ) );

			foreach ( $worker_searches as $worker_query ) {
				foreach ( $templates as $template ) {
					if ( $requests >= $request_budget ) {
						$errors[] = 'Stopped external browser worker search because the discovery request budget was reached.';
						return;
					}

					$search_url = $this->build_search_url( $domain, $template, $worker_query );
					if ( '' === $search_url || ! $this->url_service->is_safe_url( $search_url, $ports ) || ! $this->url_service->matches_domain( $search_url, $domain ) ) {
						continue;
					}

					$result = $this->external_worker->search( $search_url, $settings, $competitor, $product );
					if ( ! empty( $result['searched_url'] ) ) {
						$searched_urls[] = (string) $result['searched_url'];
					} else {
						$searched_urls[] = $search_url;
					}
					$requests += max( 0, (int) ( $result['request_count'] ?? 0 ) );
					$this->add_candidate_urls_with_source_score( $urls, $source_scores, $source_types, (array) ( $result['urls'] ?? array() ), 640, 'worker' );
					if ( ! empty( $result['technical_details'] ) ) {
						$errors[] = (string) $result['technical_details'];
					}
				}
			}
		};
		$run_name_search = function () use ( $settings, $name_queries, $templates, $domain, $ports, $product, $request_budget, &$urls, &$source_scores, &$source_types, &$requests, &$errors, &$searched_urls, &$name_search_ran ): void {
			if ( $name_search_ran || empty( $settings['discovery_name_search_enabled'] ) || empty( $name_queries ) ) {
				return;
			}

			$name_search_ran = true;
			foreach ( $name_queries as $name_query ) {
				foreach ( $templates as $template ) {
					if ( $requests >= $request_budget ) {
						$errors[] = 'Stopped product-name search because the discovery request budget was reached.';
						return;
					}
					...
					if ( $code >= 300 && $code < 400 ) {
						...
						if ( $this->is_safe_same_domain_url( $next_url, $domain, $ports ) && ! $this->looks_like_search_results_url( $next_url ) && ( $this->text_matches_product_name( $next_url, $name_query ) || $this->looks_like_redirect_product_candidate( $next_url ) ) ) {
							$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, $next_url, 360, 'name' );
					...
					foreach ( $name_candidates as $candidate ) {
						$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, (string) $candidate, 560, 'name' );
					}
					if ( empty( $name_candidates ) ) {
						...
						$this->add_candidate_urls_with_source_score( $urls, $source_scores, $source_types, (array) ( $voyado['urls'] ?? array() ), 420, 'name' );
					...
					if ( empty( $name_candidates ) && empty( $voyado['urls'] ) ) {
						...
						$this->add_candidate_urls_with_source_score( $urls, $source_scores, $source_types, (array) ( $algolia['urls'] ?? array() ), 380, 'name' );
					...
					if ( $this->text_matches_product_name( $body, $name_query ) && ! $this->looks_like_search_results_url( $search_url ) && $this->url_service->looks_like_product_url( $search_url, array(), $this->settings->get_list( 'discovery_exclude_url_patterns' ), $this->settings->get_list( 'discovery_product_url_patterns' ) ) ) {
						$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, $search_url, 360, 'name' );
					}
				}
			}
		};

		$worker_first = $this->external_worker && $this->external_worker->should_use_for_search( $competitor, $settings, false );
		$run_worker_search( false );

		if ( ! $worker_first ) {
			if ( 'name' === (string) ( $search_order[0] ?? '' ) ) {
				$run_name_search();
			}

			foreach ( $queries as $query ) {
				$query_type = 'gtin' === (string) ( $query['type'] ?? '' ) ? 'gtin' : 'sku';
				foreach ( $templates as $template ) {
					if ( $requests >= $request_budget ) {
						$errors[] = 'Stopped identifier search because the discovery request budget was reached.';
						break 2;
					}
				...
				if ( $code >= 300 && $code < 400 ) {
					...
					if ( $this->is_safe_same_domain_url( $next_url, $domain, $ports ) && ! $this->looks_like_search_results_url( $next_url ) && ( $this->text_mentions_sku( $next_url, (string) $query['value'], (string) $query['normalized'] ) || $this->looks_like_redirect_product_candidate( $next_url ) ) ) {
						$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, $next_url, 540, $query_type );
						$sku_evidence = true;
				...
				foreach ( $candidates as $candidate ) {
					$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, (string) $candidate, 520, $query_type );
					$sku_evidence = true;
				}
				$this->add_candidate_urls_with_source_score( $urls, $source_scores, $source_types, (array) ( $voyado['urls'] ?? array() ), 500, $query_type );
				$sku_evidence = $sku_evidence || ! empty( $voyado['urls'] );
				foreach ( $identifier_page_candidates as $candidate ) {
					$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, (string) $candidate, 410, $query_type );
					$sku_evidence = true;
				}
				...
				if ( empty( $candidates ) && empty( $voyado['urls'] ) && empty( $voyado['hard_failure'] ) && empty( $identifier_page_candidates ) ) {
					...
					$this->add_candidate_urls_with_source_score( $urls, $source_scores, $source_types, (array) ( $algolia['urls'] ?? array() ), 450, $query_type );
				...
				if ( $this->text_mentions_sku( $body, (string) $query['value'], (string) $query['normalized'] ) && ! $this->looks_like_search_results_url( $search_url ) && $this->url_service->looks_like_product_url( $search_url, array(), $this->settings->get_list( 'discovery_exclude_url_patterns' ), $this->settings->get_list( 'discovery_product_url_patterns' ) ) ) {
					$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, $search_url, 540, $query_type );
					$sku_evidence = true;
		...
		if ( empty( $urls ) ) {
			$run_worker_search( true );
		}

		$urls = $this->rank_candidate_urls( $urls, $product, $sku, $gtin, $source_scores );
		$winning_search_type = ! empty( $urls ) ? (string) ( $source_types[ $urls[0] ] ?? 'unknown' ) : '';
		if ( '' !== $winning_search_type && 'unknown' !== $winning_search_type ) {
			$this->record_adaptive_search_success( $competitor, $winning_search_type );
		}
		...
		return array(
			...
			'adaptive_search_order' => $search_order,
			'winning_search_type'  => $winning_search_type,
	...
	private function visible_product_card_urls_from_html( string $html, string $base_url, string $domain ): array {
		$urls = array();
		foreach ( $this->product_card_fragments_from_html( $html ) as $card_html ) {
	...
	private function candidate_links_from_html( string $html, string $base_url, string $domain ): array {
		$candidates = array();
		foreach ( $this->product_card_fragments_from_html( $html ) as $card_html ) {
			foreach ( $this->links_from_html_fragment( $card_html, $base_url, $domain ) as $candidate ) {
				$this->add_candidate_link( $candidates, $candidate['url'], $candidate['text'], $card_html, $candidate['raw'], $domain );
			}
		}
		...
		if ( preg_match_all( '#<a\b([^>]*)>(.*?)</a>#is', $html, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE ) ) {
			foreach ( $matches as $match ) {
				...
				$context_html = $this->anchor_context_html( $html, (int) $match[0][1], strlen( $tag ) );
				if ( $this->is_navigation_context( $context_html ) && ! $this->has_product_card_context( $context_html ) ) {
					continue;
				}
				$text = html_entity_decode( wp_strip_all_tags( (string) $match[2][0] ), ENT_QUOTES | ENT_HTML5, 'UTF-8' );
				$context = $this->html_fragment_text( $context_html );
		...
		if ( preg_match_all( '#<([a-z][a-z0-9:-]*)\b([^>]*)>#is', $html, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE ) ) {
			foreach ( $matches as $match ) {
				...
				$context_html = $this->anchor_context_html( $html, (int) $match[0][1], strlen( $tag_html ) );
				if ( $this->is_navigation_context( $context_html ) && ! $this->has_product_card_context( $context_html ) ) {
					continue;
				}
				$context = $this->html_fragment_text( $context_html );
	...
	/**
	 * Extract product-card fragments from common Magento, WooCommerce, Shopify,
	 * Voyado-rendered, Algolia-rendered and Jollyroom-style result grids.
	 *
	 * @return array<int,string>
	 */
	private function product_card_fragments_from_html( string $html ): array {
		$fragments = array();
		$patterns  = array(
			'#<product-card\b[^>]*>.*?</product-card>#is',
			'#<(?:article|li|div|product-card|product-tile)\b[^>]*(?:product-item|js-product-item|product-card|product-tile|product-grid-item|product-list-item|product-item-info|product-item-details|product-info|woocommerce-LoopProduct|type-product|data-product-impression|data-product-id|data-product-sku|data-product-name|itemtype=["\'][^"\']*schema\.org/Product)[^>]*>.*?</(?:article|li|div|product-card|product-tile)>#is',
			'#<(?:article|li|div)\b[^>]*\bclass=["\'][^"\']*(?:^|\s)product(?:\s|$)[^"\']*["\'][^>]*>.*?</(?:article|li|div)>#is',
		);
		...
		foreach ( $patterns as $pattern ) {
			...
			foreach ( $matches[0] as $fragment ) {
				...
				if ( $this->has_product_card_context( $fragment ) ) {
					$fragments[ md5( $fragment ) ] = $fragment;
				}
			}
		}
	 ...
	 * @return array<int,array{url:string,text:string,context:string,raw:string}>
	 */
	private function links_from_html_fragment( string $html, string $base_url, string $domain ): array {
		$candidates = array();
		if ( preg_match_all( '#<a\b([^>]*)>(.*?)</a>#is', $html, $matches, PREG_SET_ORDER ) ) {
			foreach ( $matches as $match ) {
				...
				$attrs = $this->html_attributes_from_tag( $tag );
				$href  = (string) ( $attrs['href'] ?? '' );
				$text  = html_entity_decode( wp_strip_all_tags( (string) $match[2] ), ENT_QUOTES | ENT_HTML5, 'UTF-8' );
				$this->add_candidate_link( $candidates, $this->url_service->resolve( $href, $base_url ), $text, $html, $href, $domain );
			}
		}

		if ( preg_match_all( '#<([a-z][a-z0-9:-]*)\b([^>]*)>#is', $html, $matches, PREG_SET_ORDER ) ) {
			foreach ( $matches as $match ) {
				...
				foreach ( $this->html_attributes_from_tag( $tag_html ) as $name => $value ) {
					if ( ! $this->is_url_bearing_attribute( $name ) ) {
						continue;
					}
					foreach ( $this->urls_from_attribute_value( $value, $base_url ) as $url ) {
						$this->add_candidate_link( $candidates, $url, '', $html, $value, $domain );
					}
				}
			}
		}
	 ...
	 * @param array<string,string>      $source_types URL source type map.
	 * @param array<int,mixed>          $candidates Ranked candidate URLs from a search source.
	 */
	private function add_candidate_urls_with_source_score( array &$urls, array &$source_scores, array &$source_types, array $candidates, int $base_score, string $source_type ): void {
		foreach ( array_values( $candidates ) as $index => $candidate ) {
			$this->add_candidate_url_with_source_score( $urls, $source_scores, $source_types, (string) $candidate, max( 1, $base_score - ( $index * 80 ) ), $source_type );
		}
	}
	 ...
	 * @param array<string,string> $source_types URL source type map.
	 */
	private function add_candidate_url_with_source_score( array &$urls, array &$source_scores, array &$source_types, string $url, int $score, string $source_type ): void {
		$url = $this->url_service->normalize( $url );
		if ( '' === $url ) {
			return;
		}
		...
		if ( ! isset( $source_types[ $url ] ) || $score >= (int) ( $source_scores[ $url ] ?? 0 ) ) {
			$source_types[ $url ] = $this->normalize_search_type( $source_type );
		}
	}
	...
	private function anchor_context_text( string $html, int $offset, int $anchor_length ): string {
		return $this->html_fragment_text( $this->anchor_context_html( $html, $offset, $anchor_length ) );
	}

	private function anchor_context_html( string $html, int $offset, int $anchor_length ): string {
		$start = max( 0, $offset - 900 );
		$length = $anchor_length + 1800;
		return (string) substr( $html, $start, $length );
	}

	private function html_fragment_text( string $html ): string {
		return html_entity_decode( wp_strip_all_tags( $html ), ENT_QUOTES | ENT_HTML5, 'UTF-8' );
	}

	private function is_navigation_context( string $html ): bool {
		return (bool) preg_match( '#<(?:nav|header|footer|aside)\b|class=["\'][^"\']*(?:menu|navigation|navbar|breadcrumb|sidebar|header|footer)[^"\']*["\']#i', $html );
	}
	...
	private function candidate_match_score( string $url, string $context, string $product_name ): int {
		...
		$product_colors   = $this->color_tokens_from_canonical_text( $product_words );
		$candidate_colors = $this->color_tokens_from_canonical_text( $candidate_words );
		if ( ! empty( $product_colors ) && ! empty( $candidate_colors ) && empty( array_intersect( $product_colors, $candidate_colors ) ) ) {
			$score -= 30;
		}
	 ...
	 * @return array<int,string>
	 */
	private function color_tokens_from_canonical_text( string $text ): array {
		$colors = array();
		foreach ( array_keys( $this->color_term_groups() ) as $color ) {
	...
	private function looks_like_redirect_product_candidate( string $url ): bool {
		...
		if ( '' === $path || '/' === $path || preg_match( '#/(?:catalogsearch|search|sok|category|kategori|brand|brands|merke|merker|varemerker|collection|collections|blog|news|nyheter)(?:/|$)#i', $path ) ) {
			return false;
		}
	...
	private function looks_like_listing_or_category_url( string $url ): bool {
		...
		return (bool) preg_match( '#/(?:product-category|category|kategori|collections?|brand|brands|merke|merker|varemerker|tag|product-tag|blog|news|nyheter)(?:/|$)#i', $path );
	}
	...
	private function looks_like_product_candidate_link( string $url, string $text, bool $broad_listing_page ): bool {
		if ( $this->looks_like_search_results_url( $url ) ) {
			return false;
		}
		...
		if ( preg_match( '#/(?:category|kategori|brand|brands|merke|merker|varemerker|collection|collections|search|sok|blog|news|nyheter)(?:/|$)#i', $path ) ) {
			return false;
		}
	...
	private function has_product_card_context( string $text ): bool {
		...
		return str_contains( $raw, 'product-item' )
			...
			|| str_contains( $raw, 'product-card' )
			|| str_contains( $raw, 'product-tile' )
			|| str_contains( $raw, 'product-grid-item' )
			|| str_contains( $raw, 'product-list-item' )
			|| str_contains( $raw, 'product-item-info' )
			|| str_contains( $raw, 'woocommerce-loopproduct' )
			|| str_contains( $raw, 'type-product' )
			|| str_contains( $raw, 'itemtype="https://schema.org/product' )
			|| str_contains( $raw, "itemtype='https://schema.org/product" )
			|| str_contains( $raw, 'data-product-id=' )
			|| str_contains( $raw, 'data-product-sku=' )
			|| str_contains( $raw, 'data-product-name=' )
			|| str_contains( $raw, 'data-name=' )
			|| str_contains( $raw, 'data-product-impression' )
			|| str_contains( $raw, 'standard-product-price' )
			|| str_contains( $raw, 'price-box' )
			|| str_contains( $raw, 'price-final_price' )
			|| str_contains( $normalized, 'se produkt' )
			|| str_contains( $normalized, 'legg i handlekurv' )
			|| str_contains( $normalized, 'add to cart' )
	 ...
	 */
	private function adaptive_search_order( array $competitor, array $identifier_queries, array $name_queries ): array {
		$available = array();
		foreach ( $identifier_queries as $query ) {
			$type = $this->normalize_search_type( (string) ( $query['type'] ?? '' ) );
			if ( in_array( $type, array( 'sku', 'gtin' ), true ) && ! in_array( $type, $available, true ) ) {
				$available[] = $type;
			}
		}
		if ( ! empty( $name_queries ) ) {
			$available[] = 'name';
		}
		if ( empty( $available ) ) {
			return array();
		...
		$override = $this->adaptive_search_order_override( $competitor );
		if ( ! empty( $override ) ) {
			return $this->merge_search_order( $override, $available );
		}

		$stats = $this->get_adaptive_search_stats();
		$key   = $this->adaptive_strategy_key( $competitor );
		$row   = is_array( $stats[ $key ] ?? null ) ? $stats[ $key ] : array();
		$base  = array( 'sku' => 30, 'gtin' => 20, 'name' => 10 );
		$scores = array();
		foreach ( $available as $type ) {
			$wins = absint( $row[ $type ]['successes'] ?? 0 );
			$scores[ $type ] = ( $wins * 100 ) + ( $base[ $type ] ?? 0 );
		}

		$order = $available;
		usort(
			$order,
			static function ( string $left, string $right ) use ( $scores, $base ): int {
				if ( $scores[ $left ] === $scores[ $right ] ) {
					return ( $base[ $right ] ?? 0 ) <=> ( $base[ $left ] ?? 0 );
				}
	 ...
	 * @return array<int,string>
	 */
	private function adaptive_search_order_override( array $competitor ): array {
		$notes = json_decode( (string) ( $competitor['notes'] ?? '' ), true );
		if ( ! is_array( $notes ) ) {
			return array();
		}

		$raw = $notes['preferred_search_order'] ?? $notes['adaptive_search_order'] ?? array();
		$items = is_array( $raw ) ? $raw : explode( ',', (string) $raw );
		$order = array();
		foreach ( $items as $item ) {
			$type = $this->normalize_search_type( (string) $item );
			if ( in_array( $type, array( 'sku', 'gtin', 'name' ), true ) && ! in_array( $type, $order, true ) ) {
				$order[] = $type;
			}
		}
	...
	/**
	 * @param array<int,string> $preferred Preferred order.
	 * @param array<int,string> $available Available query families.
	 * @return array<int,string>
	 */
	private function merge_search_order( array $preferred, array $available ): array {
		$order = array();
		foreach ( $preferred as $type ) {
			if ( in_array( $type, $available, true ) && ! in_array( $type, $order, true ) ) {
				$order[] = $type;
			}
		}
		foreach ( $available as $type ) {
	...
	/**
	 * @param array<int,array{type:string,value:string,normalized:string}> $queries Identifier queries.
	 * @param array<int,string> $search_order Adaptive search order.
	 * @return array<int,array{type:string,value:string,normalized:string}>
	 */
	private function order_identifier_queries( array $queries, array $search_order ): array {
		$rank = array_flip( $search_order );
		usort(
			$queries,
			function ( array $left, array $right ) use ( $rank ): int {
				$left_type  = $this->normalize_search_type( (string) ( $left['type'] ?? '' ) );
				$right_type = $this->normalize_search_type( (string) ( $right['type'] ?? '' ) );

				return ( $rank[ $left_type ] ?? 99 ) <=> ( $rank[ $right_type ] ?? 99 );
			}
		);
	...
	private function record_adaptive_search_success( array $competitor, string $source_type ): void {
		$source_type = $this->normalize_search_type( $source_type );
		if ( ! in_array( $source_type, array( 'sku', 'gtin', 'name', 'worker' ), true ) ) {
			return;
		}
		...
		$stats = $this->get_adaptive_search_stats();
		$row = is_array( $stats[ $key ] ?? null ) ? $stats[ $key ] : array();
		foreach ( array( 'sku', 'gtin', 'name', 'worker' ) as $type ) {
			if ( ! isset( $row[ $type ] ) || ! is_array( $row[ $type ] ) ) {
				$row[ $type ] = array( 'successes' => 0 );
			}
		}

		$row[ $source_type ]['successes'] = min( 1000, absint( $row[ $source_type ]['successes'] ?? 0 ) + 1 );
		...
		if ( count( $stats ) > 100 ) {
			uasort(
				$stats,
				static fn( $left, $right ): int => strcmp( (string) ( $right['updated_at'] ?? '' ), (string) ( $left['updated_at'] ?? '' ) )
			);
			$stats = array_slice( $stats, 0, 100, true );
		}

		update_option( self::STRATEGY_OPTION, $stats, false );
	}

	/**
	 * @return array<string,mixed>
	 */
	private function get_adaptive_search_stats(): array {
		$stats = get_option( self::STRATEGY_OPTION, array() );

		return is_array( $stats ) ? $stats : array();
	}

	private function adaptive_strategy_key( array $competitor ): string {
		$id = absint( $competitor['id'] ?? 0 );
		if ( $id > 0 ) {
			return 'id:' . $id;
		}
		...
		return '' !== $domain ? 'domain:' . strtolower( $domain ) : '';
	}

	private function normalize_search_type( string $type ): string {
		$type = strtolower( trim( $type ) );
		if ( in_array( $type, array( 'ean', 'gtin13', 'gtin14', 'barcode' ), true ) ) {
			return 'gtin';
		}
		if ( in_array( $type, array( 'title', 'model', 'product_name' ), true ) ) {
			return 'name';
		}

		return in_array( $type, array( 'sku', 'gtin', 'name', 'worker' ), true ) ? $type : 'unknown';
	}

	private function discovery_request_budget( int $request_limit, int $identifier_count, int $name_query_count ): int {
		$query_families = max( 1, $identifier_count + $name_query_count );

		return max( 1, min( 50, $request_limit * $query_families ) );
	}
	...
	private function synonym_product_name_queries( string $name ): array {
		...
		$queries = array();
		foreach ( array_merge( $this->retail_term_groups(), $this->color_term_groups() ) as $canonical => $terms ) {
	...
	public function build_search_url( string $domain, string $template, string $sku ): string {
		...
		$url   = str_replace( array( '{domain}', '{sku}', '{query}', '{gtin}', '{ean}', '%s' ), array( $domain, $value, $value, $value, $value, $value ), $template );
		if ( false === strpos( $url, '://' ) ) {
