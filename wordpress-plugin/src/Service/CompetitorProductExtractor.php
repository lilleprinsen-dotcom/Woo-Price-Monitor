...
class CompetitorProductExtractor {
	 ...
	 * Constructor.
	 */
	public function __construct( DiscoveryUrlService $url_service, DiscoverySettings $settings, ?ExternalBrowserWorkerClient $external_worker = null ) {
	...
	public function test_url( string $url, array $competitor = array() ): array {
		...
		if ( $this->external_worker && $this->external_worker->should_use_for_product( $competitor, $settings, false ) ) {
			$worker_result = $this->external_worker->product( $url, $settings, $competitor );
			if ( ! empty( $worker_result['success'] ) || $this->external_worker->is_always_mode( $competitor ) || ! empty( $competitor['requires_javascript'] ) ) {
