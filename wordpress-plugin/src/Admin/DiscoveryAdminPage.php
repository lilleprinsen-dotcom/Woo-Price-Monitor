...
class DiscoveryAdminPage {
	...
	public function register_menu(): void {
		add_submenu_page( null, __( 'Price Monitor legacy route', 'lilleprinsen-price-monitor' ), __( 'Price Monitor legacy route', 'lilleprinsen-price-monitor' ), 'manage_woocommerce', 'lpm-competitor-prices', array( $this, 'render' ) );
	}
	...
	public function render(): void {
		...
		$url = add_query_arg( array( 'page' => AdminPage::SLUG, 'tab' => $redirect_tab ), admin_url( 'admin.php' ) );
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Price Monitor has moved', 'lilleprinsen-price-monitor' ); ?></h1>
			<p><?php esc_html_e( 'Competitor discovery now lives inside the unified Price Monitor app.', 'lilleprinsen-price-monitor' ); ?></p>
			<p><a class="button button-primary" href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Open Price Monitor', 'lilleprinsen-price-monitor' ); ?></a></p>
		</div>
		<?php
	}
	...
	private function render_tabs( string $current ): void {
		$tabs = array(
			...
			'suggestions' => __( 'Suggestions', 'lilleprinsen-price-monitor' ),
	...
	private function render_products(): void {
		...
		?>
		<h2><?php esc_html_e( 'Products', 'lilleprinsen-price-monitor' ); ?></h2>
	...
	private function render_suggestions(): void {
		...
		?>
		<h2><?php esc_html_e( 'Suggestions', 'lilleprinsen-price-monitor' ); ?></h2>
	...
	private function handle_run_small_discovery(): void {
		...
		if ( $this->job->enqueue_manual_batch( $competitor_id ) ) {
			$this->set_notice( __( 'Find matches has been queued. Check Suggestions in a minute; approved matches will be added to regular price monitoring.', 'lilleprinsen-price-monitor' ) );
		} else {
	...
	private function render_admin_test_status_panel( int $selected_products, array $active_competitors, array $settings ): void {
		...
		<div style="background:#fff;border:1px solid #ccd0d4;padding:14px;margin:12px 0 18px;">
			...
			<div style="display:flex;gap:8px;flex-wrap:wrap;">
				...
				<?php $this->status_badge_text( __( 'Scheduled discovery', 'lilleprinsen-price-monitor' ), ! empty( $settings['discovery_enabled'] ) ? sprintf( __( 'Every %d day(s)', 'lilleprinsen-price-monitor' ), (int) $settings['discovery_schedule_interval_days'] ) : __( 'Disabled', 'lilleprinsen-price-monitor' ), ! empty( $settings['discovery_enabled'] ) ? 'warning' : 'muted' ); ?>
				<?php $this->status_badge_text( __( 'Scheduled price checks', 'lilleprinsen-price-monitor' ), ! empty( $settings['scheduled_checks_enabled'] ) ? sprintf( __( 'Every %d hour(s)', 'lilleprinsen-price-monitor' ), (int) $settings['scheduled_check_interval_hours'] ) : __( 'Disabled', 'lilleprinsen-price-monitor' ), ! empty( $settings['scheduled_checks_enabled'] ) ? 'warning' : 'muted' ); ?>
