...
use Lilleprinsen\PriceMonitor\Service\VisualProductMatchService;
...
lpm_run_tests(
	...
	array(
		...
		'Match scoring caps exact identifiers when price is wildly outside range' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 19, 'product_id' => 190, 'variation_id' => 0, 'sku' => 'ABC-999', 'gtin' => '', 'mpn' => '', 'brand' => 'Easy Brand', 'price' => 1000, 'normalized_sku' => 'ABC999', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Easy Brand Product', 'brand' => 'Easy Brand', 'sale_price' => 199, 'normalized_sku' => 'ABC999', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'exact_sku', $result['match_type'], 'Exact SKU should still be recorded as the positive identifier evidence.' );
			lpm_assert_same( 'Low confidence', $result['confidence_label'], 'Wild price sanity failure should prevent high confidence even with exact SKU.' );
			lpm_assert_contains( 'Price sanity reject', $result['explanation'], 'Explanation should warn that the candidate price is wildly outside range.' );
		},
		...
		'Match scoring adds positive evidence when candidate price is sane' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 20, 'product_id' => 200, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Nordic Baby', 'price' => 1000, 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Nordic Baby Stroller Model X', 'brand' => 'Nordic Baby', 'sale_price' => 929, 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$GLOBALS['lpm_test_titles'][200] = 'Nordic Baby Stroller Model X';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'Medium confidence', $result['confidence_label'], 'Sane price should support the existing brand/title evidence.' );
			lpm_assert_contains( 'Price sanity check passed', $result['explanation'], 'Explanation should show price sanity as positive evidence.' );
		},
		'Match scoring rejects non-identifier candidates with absurd low price' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 21, 'product_id' => 210, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Nordic Baby', 'price' => 1000, 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Nordic Baby Stroller Model X', 'brand' => 'Nordic Baby', 'sale_price' => 199, 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$GLOBALS['lpm_test_titles'][210] = 'Nordic Baby Stroller Model X';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( array(), $result, 'Absurd non-identifier candidate prices should be rejected instead of creating a weak suggestion.' );
		},
		...
		'Match scoring warns when candidate URL is a scraped category page' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 18, 'product_id' => 180, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Thule', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Thule Chariot Sport 2 double black', 'brand' => 'Thule', 'url' => 'https://competitor.no/category/thule-chariot', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$GLOBALS['lpm_test_titles'][180] = 'Thule Chariot Sport 2 double black';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'Low confidence', $result['confidence_label'], 'Category/search page anomalies should force low-confidence review.' );
			lpm_assert_contains( 'candidate URL looks like a search, category', $result['explanation'], 'Explanation should warn about scraped category pages.' );
		},
		...
		'Match scoring understands grey and grå as the same color' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 13, 'product_id' => 130, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Cybex', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Cybex Sirona T i-Size Plus Mirage Grå', 'brand' => 'Cybex', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$GLOBALS['lpm_test_titles'][130] = 'Cybex Sirona T i-Size Plus Mirage Grey';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'Medium confidence', $result['confidence_label'], 'Grey/grå color vocabulary should support a medium-confidence evidence match when brand and model also match.' );
			lpm_assert_contains( 'Grey terminology matches', $result['explanation'], 'Explanation should show grey vocabulary evidence.' );
			lpm_assert_contains( 'Color/variant appears to match', $result['explanation'], 'Grey/grå should not be treated as a color mismatch.' );
		},
		'Match scoring warns on taupe beige mismatch' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 14, 'product_id' => 140, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Thule', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Thule Chariot Sport 2 double Beige', 'brand' => 'Thule', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$GLOBALS['lpm_test_titles'][140] = 'Thule Chariot Sport 2 double Tinted Taupe';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'Low confidence', $result['confidence_label'], 'Taupe versus beige should be visible as a risky variant mismatch.' );
			lpm_assert_contains( 'Color/variant mismatch', $result['explanation'], 'Explanation should warn about taupe/beige mismatch.' );
		},
		'Match scoring understands mocha and mokka as the same color' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 15, 'product_id' => 150, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Stokke', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$discovered = (object) array( 'title' => 'Stokke Tripp Trapp Stol Mokka', 'brand' => 'Stokke', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '' );
			$GLOBALS['lpm_test_titles'][150] = 'Stokke Tripp Trapp Chair Mocha';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'Medium confidence', $result['confidence_label'], 'Mocha/mokka should count as matching color vocabulary with brand and model evidence.' );
			lpm_assert_contains( 'Mocha terminology matches', $result['explanation'], 'Explanation should show mocha vocabulary evidence.' );
		},
		...
		'Visual image embeddings add positive match evidence' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 16, 'product_id' => 160, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Bugaboo', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '', 'product_image_url' => 'https://store.test/bugaboo.jpg', 'image_embedding' => array( 0.9, 0.2, 0.1, 0.8, 0.7, 0.1, 0.3, 0.6 ) );
			$discovered = (object) array( 'title' => 'Bugaboo Donkey 5 Black', 'brand' => 'Bugaboo', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '', 'image_url' => 'https://competitor.test/bugaboo.jpg', 'image_embedding' => array( 0.91, 0.19, 0.1, 0.82, 0.69, 0.12, 0.29, 0.58 ) );
			$GLOBALS['lpm_test_titles'][160] = 'Bugaboo Donkey 5 Black';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_contains( 'Product images are visually very similar', $result['explanation'], 'Visual embeddings should be visible as positive evidence.' );
			lpm_assert_true( 'High confidence' !== $result['confidence_label'], 'Visual evidence must not create high confidence without exact identifiers.' );
		},
		'Visual image embeddings warn on visual mismatch' => static function () use ( $matcher ): void {
			$product = (object) array( 'id' => 17, 'product_id' => 170, 'variation_id' => 0, 'sku' => '', 'gtin' => '', 'mpn' => '', 'brand' => 'Thule', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '', 'product_image_url' => 'https://store.test/thule.jpg', 'image_embedding' => array( 1, 0, 1, 0, 1, 0, 1, 0 ) );
			$discovered = (object) array( 'title' => 'Thule Chariot Sport 2 double black', 'brand' => 'Thule', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '', 'image_url' => 'https://competitor.test/wrong.jpg', 'image_embedding' => array( 0, 1, 0, 1, 0, 1, 0, 1 ) );
			$GLOBALS['lpm_test_titles'][170] = 'Thule Chariot Sport 2 double black';
			$result = $matcher->score_match( $product, $discovered );

			lpm_assert_same( 'Low confidence', $result['confidence_label'], 'Visual mismatch should force manual low-confidence review.' );
			lpm_assert_contains( 'Visual image mismatch', $result['explanation'], 'Visual mismatch should be visible to the admin.' );
		},
		'Visual matcher skips cleanly when images are missing' => static function (): void {
			$visual = new VisualProductMatchService();
			$result = $visual->compare( (object) array( 'product_id' => 1 ), (object) array( 'title' => 'Competitor' ) );

			lpm_assert_same( 'missing_image', $result['status'], 'Missing images should skip visual matching without warnings.' );
			lpm_assert_same( 0, $result['score'], 'Missing images should not change score.' );
		},
		...
		'Rejected suggestion fingerprints ignore price noise until candidate identity changes' => static function () use ( $matcher ): void {
			...
			$discovered = (object) array( 'competitor_id' => 4, 'url_hash' => 'url-hash', 'content_hash' => 'same-content', 'title' => 'Thule Bag Black', 'brand' => 'Thule', 'normalized_sku' => '', 'normalized_gtin' => '', 'normalized_mpn' => '', 'image_url' => 'https://competitor.no/media/thule-bag.jpg' );
			...
			$content_changed = $method->invoke( $matcher, $product, $discovered, 'title_only' );
			$discovered->title = 'Different Product Black';
			$identity_changed = $method->invoke( $matcher, $product, $discovered, 'title_only' );

			lpm_assert_same( $first, $second, 'Same candidate identity should keep the same fingerprint, so rejected suggestions do not immediately reappear.' );
			lpm_assert_same( $first, $content_changed, 'Noisy page content or price changes should not create a new rejected-suggestion fingerprint.' );
			lpm_assert_true( $first !== $identity_changed, 'Changed candidate identity should get a new fingerprint for review.' );
		},
