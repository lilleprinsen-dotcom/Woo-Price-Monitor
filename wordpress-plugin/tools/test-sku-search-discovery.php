...
lpm_run_tests(
	...
	array(
		...
		'Discovery request budget covers SKU EAN name and worker searches without overrunning' => static function () use ( $sku_search ): void {
			update_option(
				Settings::OPTION_NAME,
				array(
					'discovery_name_search_enabled'      => 1,
					'discovery_search_urls_per_sku'      => 1,
					'discovery_sku_search_url_templates' => '?s={query}, search?q={query}, catalogsearch/result/?q={query}',
					'discovery_product_url_patterns'     => 'produkt,product,p',
					'discovery_exclude_url_patterns'     => 'cart,checkout,account,login,filter,wp-admin,add-to-cart',
				)
			);
			...
			preg_match( '/Discovery request budget: ([0-9]+)/', $result['technical_details'], $budget_match );
			$budget = (int) ( $budget_match[1] ?? 0 );

			lpm_assert_true( $budget > 0, 'Diagnostics should show the bounded request budget.' );
			lpm_assert_true( $result['request_count'] <= $budget, 'Discovery should not exceed the shared request budget for identifiers and name queries.' );
		},
		'Adaptive search strategy can try product name before SKU for a competitor' => static function () use ( $sku_search ): void {
			update_option(
				Settings::OPTION_NAME,
				array(
					'discovery_name_search_enabled'      => 1,
			...
			update_option(
				'lpm_discovery_search_strategy_stats',
				array(
					'id:77' => array(
						'sku'        => array( 'successes' => 0 ),
						'gtin'       => array( 'successes' => 0 ),
						'name'       => array( 'successes' => 3 ),
			...
			$GLOBALS['lpm_test_http_responses'] = array(
				'https://competitor.no/?s=Thule%20Chariot%20Sport%202%20double%20midnight%20black' => array(
					'body' => '<div class="product-card"><a href="/produkt/thule-chariot-sport-2-double-midnight-black">Thule Chariot Sport 2 double midnight black</a></div>',
				),
				'https://competitor.no/?s=10201031' => array(
					'body' => '<html><body>No SKU result</body></html>',
				),
			);

			$result = $sku_search->discover_for_product(
				array( 'id' => 77, 'domain' => 'competitor.no', 'enabled' => 1 ),
				(object) array(
					'id'             => 81,
					'product_id'     => 181,
					'sku'            => '10201031',
			...
			unset( $GLOBALS['lpm_test_http_responses'] );
			delete_option( 'lpm_discovery_search_strategy_stats' );

			lpm_assert_same( 'https://competitor.no/?s=Thule%20Chariot%20Sport%202%20double%20midnight%20black', $result['searched_urls'][0] ?? '', 'Learned name strategy should make product-name search run first.' );
			lpm_assert_same( 'name', $result['winning_search_type'], 'The winning search type should be recorded.' );
			lpm_assert_same( 4, (int) $stats['id:77']['name']['successes'], 'Successful name discovery should increment the competitor strategy profile.' );
			lpm_assert_contains( 'Adaptive search order: name, sku', $result['technical_details'], 'Diagnostics should show the chosen adaptive order.' );
		},
		...
		'Product card extraction supports Magento WooCommerce and Shopify card markup' => static function () use ( $sku_search ): void {
			$magento = '<ol class="products list items product-items"><li class="item product product-item"><div class="product-item-info" data-container="product-grid"><a class="product photo product-item-photo" href="/bag-thule-black"><span class="product-image-wrapper"></span></a><strong class="product name product-item-name"><a class="product-item-link" href="/bag-thule-black">Bag, Thule, Black</a></strong><div class="price-box price-final_price"><span class="price">kr 3 499,00</span></div></div></li><li><a href="/merker/thule">Thule brand</a></li></ol>';
			$woo     = '<ul class="products"><li class="product type-product post-10"><a class="woocommerce-LoopProduct-link" href="/produkt/besafe-beyond-2-360-black-softbreeze"><h2>Besafe Beyond 2 360 Black SoftBreeze</h2><span class="price">6 399 kr</span></a></li><li><a href="/product-category/bilstol/">Bilstol</a></li></ul>';
			$shopify = '<product-card class="card"><a href="/products/thule-chariot-sport-2-double-midnight-black"><span class="product-card__title">Thule Chariot Sport 2 Double Midnight Black</span><span class="price">14 599 kr</span></a></product-card><nav><a href="/collections/thule">Thule</a></nav>';

			lpm_assert_same( array( 'https://www.babycare.no/bag-thule-black' ), $sku_search->product_candidate_urls_from_html( $magento, 'https://www.babycare.no/catalogsearch/result/?q=20110754&origin=ORGANIC', 'babycare.no', true ), 'Magento product cards should queue product URLs and skip brand links.' );
			lpm_assert_same( array( 'https://competitor.no/produkt/besafe-beyond-2-360-black-softbreeze' ), $sku_search->product_candidate_urls_from_html( $woo, 'https://competitor.no/?s=besafe', 'competitor.no', true ), 'WooCommerce product cards should queue loop product URLs and skip categories.' );
			lpm_assert_same( array( 'https://shop.no/products/thule-chariot-sport-2-double-midnight-black' ), $sku_search->product_candidate_urls_from_html( $shopify, 'https://shop.no/search?q=thule', 'shop.no', true ), 'Shopify product-card elements should queue /products/ URLs and skip collections.' );
		},
		'Search result URLs are not treated as product cards even with product-like text' => static function () use ( $sku_search ): void {
			$html = '<article class="product"><a href="/?s=Besafe%20Beyond%202%20360%20Black%20SoftBreeze">Besafe Beyond 2 360 Black SoftBreeze</a><span>6 399 kr</span></article><article class="product"><a href="/produkt/besafe-beyond-2-360-black-softbreeze">Besafe Beyond 2 360 Black SoftBreeze</a><span>6 399 kr</span></article>';
			$urls = $sku_search->product_candidate_urls_from_html( $html, 'https://www.ohdearbaby.no/?post_type=product&s=Besafe%20Beyond%202%20360', 'ohdearbaby.no', true );

			lpm_assert_same( array( 'https://www.ohdearbaby.no/produkt/besafe-beyond-2-360-black-softbreeze' ), $urls, 'Search-page URLs with s/query parameters must not be queued as final product pages.' );
		},
		'Jollyroom extraction ignores navigation when product cards are present' => static function () use ( $sku_search ): void {
			$html = '<header><nav><a href="/varemerker/reima">Reima</a><a href="/gaver/gaver-til-0-2-aringer">Gaver</a></nav></header>'
				. '<article class="product-item js-product-item" data-name="BeSafe Beyond 2 360 Bilstol, Black Soft Breeze" data-currency="NOK">'
				. '<a href="/bilstoler/bilstoler/vendbare/besafe-beyond-2-360-bilstol-black-soft-breeze"><img title="BeSafe Beyond 2 360 Bilstol, Black Soft Breeze"></a>'
				. '<div class="product-info"><a href="/bilstoler/bilstoler/vendbare/besafe-beyond-2-360-bilstol-black-soft-breeze"><h3>BeSafe Beyond 2 360 Bilstol, Black Soft Breeze</h3></a><span data-cy="standard-product-price" content="6399">6 399 kr</span></div></article>';
			$urls = $sku_search->name_matched_urls_from_html( $html, 'https://www.jollyroom.no/search?text=Besafe%20Beyond%202%20360%20Black%20SoftBreeze', 'Besafe Beyond 2 360 Black SoftBreeze', 'jollyroom.no' );

			lpm_assert_same( array( 'https://www.jollyroom.no/bilstoler/bilstoler/vendbare/besafe-beyond-2-360-bilstol-black-soft-breeze' ), $urls, 'Jollyroom navigation and brand URLs should not crowd out the actual product-card URL.' );
		},
		...
		'Product-name search prepares broader color vocabulary variants' => static function () use ( $sku_search ): void {
			update_option(
				Settings::OPTION_NAME,
				array(
					'discovery_name_search_enabled'      => 1,
			...
			$GLOBALS['lpm_test_http_responses'] = array(
				'https://competitor.no/?s=CYBEX-ANTH' => array(
					'body' => '<a href="/produkt/other">Other</a>',
				),
				'https://competitor.no/?s=Cybex%20Sirona%20T%20i%20Size%20Plus%20Anthracite%20Mesh' => array(
					'body' => '<a href="/produkt/other">Other</a>',
				),
				'https://competitor.no/?s=cybex%20sirona%20t%20i%20size%20plus%20antrasitt%20mesh' => array(
					'body' => '<article><a href="/produkt/cybex-sirona-t-i-size-plus-antrasitt-mesh"><h2>Cybex Sirona T i-Size Plus Antrasitt Mesh</h2><span>6 399 kr</span></a></article>',
				),
			);
			...
			$result = $sku_search->discover_for_product(
				...
				(object) array(
					...
					'product_id'     => 202,
					'sku'            => 'CYBEX-ANTH',
					'normalized_sku' => 'cybexanth',
					'product_name'   => 'Cybex Sirona T i-Size Plus Anthracite Mesh',
				)
			);
			unset( $GLOBALS['lpm_test_http_responses'] );

			lpm_assert_true( $result['success'], 'Color synonym name variants should be searched when exact title wording finds nothing.' );
			lpm_assert_true( in_array( 'cybex sirona t i size plus antrasitt mesh', $result['searched_names'], true ), 'Prepared name queries should include the Norwegian Antrasitt synonym.' );
			lpm_assert_same( array( 'https://competitor.no/produkt/cybex-sirona-t-i-size-plus-antrasitt-mesh' ), $result['urls'], 'The broader color synonym search result should be queued.' );
		},
